javascript: (function () {
  ...
  fileInput.addEventListener("change", function () {
    ...
    reader.onload = function (e) {
      ...
      parsed.forEach(({ hla, mfi, unacceptable }) => {
        const input = Array.from(document.querySelectorAll('input[type="hidden"][id="HLA"]')).find(
          (inp) => inp.value.toLowerCase() === hla.toLowerCase()
        );
        if (!input) return;
  ...
  addDownloadButton(); // ← This is now called immediately
